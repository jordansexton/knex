...
(0, _lodash.assign)(QueryCompiler_PG.prototype, {
  ...
  insert: function insert() {
    ...
    var _single = this.single,
      onConflict = _single.onConflict,
      returning = _single.returning;
  ...
  _onConflict: function _onConflict(value) {
    if (!value) {
      return '';
    }
    ...
    var sql = ' on conflict (' + this.formatter.columnize(columns) + ') do ';
    if (updates) {
      var updateData = this._prepUpdate(updates);
